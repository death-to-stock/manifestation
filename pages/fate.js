...
export default function Fate() {
    ...
    if (selected) {
      ...
      const generateVideo = async () => {
        try {
          ...
          saasco.track("Video Generation Started", {
            selectedManifestations: selectedItems.map((item) =>
          ...
          const audioSources = await Promise.all(
            selectedItems.map(async (audioFile) => {
              ...
              setProgress(
                5 +
                  Math.round((filesLoaded / totalFiles) * audioDecodingProgress)
              );
          ...
          recorder.onstop = () => {
            ...
            console.log(
              `💾 Video blob created: ${videoBlob.size} bytes, type: ${mimeType}`
            );
            ...
            setIsReady(true);
            setProgress(100);
            saasco.track("Video Generation Completed", {
              selectedManifestations: selectedItems.map((item) =>
                formatDisplayName(item)
              ),
              totalSelected: selectedItems.length,
          ...
          const animate = () => {
            ...
            floatingTexts.forEach((text) => {
              ...
              text.y = Math.max(
                0,
                Math.min(text.y, canvas.height - text.height)
              );
          ...
          const progressTimer = setInterval(() => {
            ...
            if (currentInterval <= totalIntervals) {
              const newProgress =
                50 +
                Math.round(
                  (currentInterval / totalIntervals) * recordingProgress
                );
          ...
          saasco.track("Video Generation Failed", {
            selectedManifestations: selectedItems.map((item) =>
              formatDisplayName(item)
            ),
            totalSelected: selectedItems.length,
  ...
  const handleDownload = () => {
    if (videoUrl) {
      saasco.track("Video Downloaded", {
        selectedManifestations: selections.map((item) =>
          formatDisplayName(item)
        ),
        totalSelected: selections.length,
        fileExtension: fileExtension,
        fileName: `dtsxtina.${fileExtension}`,
      });
      ...
      <div className={fateStyles.container}>
        ...
        <main className={fateStyles.main}>
          ...
          <div className={fateStyles.progressContainer}>
            <div
              className={fateStyles.progressBar}
  ...
  return (
    <div className={ritualStyles.container}>
      ...
      <div className={ritualStyles.header}>
        <Link
          href="/manifest"
          className={ritualStyles.backButton}
          onClick={() =>
      ...
      <main className={ritualStyles.main}>
        <div className={ritualStyles.floatingContainer}>
          {selections.map((item, index) => (
            <div
              key={index}
              className={ritualStyles.floatingText}
              style={{ "--i": index }}
            >
              {formatDisplayName(item)}
            </div>
        ...
        <div className={ritualStyles.content}>
          <h1 className={ritualStyles.title}>
            YOUR CUSTOM MANIFESTATION FREQUENCY IS READY TO USE
          </h1>
          <p className={ritualStyles.subtitle}>Ritual: 1x listen per day</p>
          {downloaded ? (
            <button
              className={ritualStyles.downloadButton}
              onClick={() => {
                saasco.track("Continue Button Clicked", {
                  source: "fate",
                  destination: "share",
                });
                router.push("/share");
